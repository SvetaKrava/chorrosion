...
A separate migration utility will allow users to import data from an existing Lidarr (C#/.NET) instance into Lidarr-Rust without requiring direct database schema compatibility.

### Rationale

- **Schema Independence**: Lidarr-Rust can evolve and improve its schema independently of the original Lidarr design.
...
- **Tool Type**: Standalone CLI binary (maybe `lidarr-migrate` crate or separate binary in workspace)
- **Connection Methods**:
...
1. Document exact schema mapping between original Lidarr and Lidarr-Rust
2. Prototype a read-only connection to sample Lidarr DB to understand data patterns
3. Build skeleton `lidarr-migrate` crate with trait-based adapters for different source versions
