...
pub async fn init_database(config: &AppConfig) -> Result<()> {
    ...
    let db_url = if config.database.url.starts_with("sqlite://") && !config.database.url.starts_with("sqlite://:memory:") {
...
mod tests {
    ...
    fn test_path_conversion_windows_style() {
        ...
        let path = Path::new("data\\lidarr.db");
